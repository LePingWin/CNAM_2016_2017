...
public class Terrain extends JComponent {
	...
	public void paint(Graphics g) {
		...
		// si on a charg� une image de fond, on l'affiche
		if (file != null && etirer) {
			// Calcule le ration pour �tirer l'image
			float scaleX = (float)getWidth()/(float)img.getWidth(null);
			float scaleY = (float)getHeight()/(float)img.getHeight(null);
			// Cr��e un op�rateur de transformation avec mes crit�res d'�tirements
			AffineTransform transform = AffineTransform.getScaleInstance(scaleX, scaleY);
			// Dessine mon image avec l'�tirement
